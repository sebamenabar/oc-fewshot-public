...
#   dataset = get_data(env_config)
  if args.to_rgb:
      data_fs_kwargs = {"to_rgb": True}
  else:
      data_fs_kwargs = {}
  dataset = get_data_fs(env_config, load_train=True, **data_fs_kwargs)
  ...
  data = get_dataiter(
    #   dataset,
      {k.replace("_fs", ""): v for k, v in dataset.items()},
  ...
  if args.data is not None:
    ...
    proto_net = None
  save_folder = os.path.join(env_config.results, env_config.dataset, f"{args.tag}_{args.seed}")
...
if __name__ == '__main__':
  ...
  parser.add_argument('--to_rgb', default=False, action="store_true")
