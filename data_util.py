...
def random_brightness(image, max_delta, impl='simclrv2', batched=False):
  """A multiplicative vs additive change of brightness."""
  if impl == 'simclrv2':
    if batched:
      factor = tf.random.uniform([image.shape[0], 1, 1, 1], tf.maximum(1.0 - max_delta, 0),
                                1.0 + max_delta)
    else:
...
def to_grayscale(image, keep_channels=True, batched=False):
    ...
    def apply_transform(i, x):
      ...
      def brightness_foo():
        if brightness == 0:
          ...
          return random_brightness(x, max_delta=brightness, impl=impl, batched=batched)
...
def crop_and_resize(image, height, width, min_object_covered=0.1, bbox=None):
